...
module.exports = {
  ...
  update: function (project, cb) {
    var libs        = project.matterLibs;
    var sassImports = ["@import '../project/matter/matter';"];

    libs.forEach(function (lib) {
      var locale = getLocale(lib);
      sassImports.push(`@import '../${locale}/matter/matter';`);
    });

    var data = sassImports.join('\n');
    data += '\n';

    fs.writeFile(`./collider/_matter.scss`, data, function (err) {
      cb(err);
    });
  },
  ...
  clone: function (lib, cb) {
    ...
    // 3. Check 'cloneUrl' is a valid Git-remote.
    q.push(function (cb) {
      isValidGitRemote(cloneUrl, cb);
    });
