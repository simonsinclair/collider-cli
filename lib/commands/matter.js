...
function matterList() {
  colliderfile.get('.', function (data) {
    printMatterInfo(JSON.parse(data));
  });
...
function matterRemove(args) {
  ...
  fs.access(`./${locale}/matter/_matter.scss`, fs.F_OK, function (err) {
    ...
    colliderfile.get('.', function (data) {
      ...
      var currentMatter = data.matter;

      // Filter Matter library to remove from Matter array.
      var newMatter = currentMatter.filter(function (matter) {
        return matter.locale !== locale;
      });

      // 1. Remove _matter.scss @import entry.
      // 2. Then remove the Matter library directory.
      // 3. And then remove the Matter Collider-file entry.
      // To do: the last operation should probably be removing the directory, so
      //        that if anything fails, we can safely run the command again
      //        without the initial 'access' check failing.
      updateMatterSass(data, './collider/_matter.scss', function () {

        rimraf(`./${locale}`, function (err) {
          if (err) throw err;
          colliderfile.update(projectName, createdTime, author, newMatter, '.');
        });
      });
    });
